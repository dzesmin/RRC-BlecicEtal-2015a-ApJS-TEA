...
out  = '/home/jasmina/Work/TEA/TEA-APJSupp-paper/TEA-paper-runs/test-compendium/CEA-TEA-TEBS/CEA-outputs'
path = out + '/*.out'

# choose species of interest with the exact names as they appear in CEA out files
# NO COMMAS in between species
specs = 'CH4 *CO H2O NH3 *N2' 

# Desired name of the results file
res_file = 'CEATEATEBS.dat'
...
filename = '/home/jasmina/Work/TEA/TEA-APJSupp-paper/TEA-paper-runs/test-compendium/CEA-TEA-TEBS/TEA/results/CEA-TEA.tea'
species = 'CH4,CO,H2O,NH3,N2'
...
# withOUT condensates
def BurrowsSharp1999(p, H2_frac, (A_C, A_O, A_N)=(2.69E-04, 4.90E-04, 6.76E-05)):

     # constancts
     R  = 1.9872041    # cal/mol/K

     # partical pressure of H2
     p_H2 = H2_frac * p
     ...
     T_array = np.arange(100.0, 4100.0, 100)
     #T_array = np.linspace(0, 4000, 40)
     K1    = np.zeros(40, dtype=np.float64)
     B_CO  = np.zeros(40, dtype=np.float64)
     B_CH4 = np.zeros(40, dtype=np.float64)
     B_H2O = np.zeros(40, dtype=np.float64)

     for i in range(len(T_array)): 
          K1[i] = np.exp((a1/T_array[i] + b1 + c1*T_array[i] + d1*T_array[i]**2 + e1*T_array[i]**3) / R / T_array[i])
          B_CO[i] = A_C + A_O + p_H2**2 / (2*K1[i]) - np.sqrt((A_C + A_O + p_H2**2 / (2*K1[i]))**2 - 4*A_C*A_O)
          B_CH4[i] = 2*A_C - B_CO[i]
          B_H2O[i] = 2*A_O - B_CO[i]
     ...
     K2    = np.zeros(40, dtype=np.float64)
     B_N2  = np.zeros(40, dtype=np.float64)
     B_NH3 = np.zeros(40, dtype=np.float64)

     for i in range(len(T_array)):   
          K2[i] = np.exp((a2/T_array[i] + b2 + c2*T_array[i] + d2*T_array[i]**2 + e2*T_array[i]**3) / (R*T_array[i]))
          B_N2[i] = A_N + p_H2**2 / (8*K2[i]) - np.sqrt((A_N + p_H2**2 / (8*K2[i]))**2 - A_N**2)
          B_NH3[i] = 2*(A_N - B_N2[i])

     return T_array, B_CO, B_CH4, B_H2O, B_N2, B_NH3

# Number density of elements involved
A_C  = 2.69E-04
A_O  = 4.90E-04
A_N  = 6.76E-05
...
# Call the function
T_array, B_CO, B_CH4, B_H2O, B_N2, B_NH3 = BurrowsSharp1999(p, 0.85, solar)

plt.semilogy(T_array, B_CO , '.', color = 'r', linewidth=1)
plt.semilogy(T_array, B_CH4, '.', color = 'k', linewidth=1, label='TEBS')
plt.semilogy(T_array, B_H2O, '.', color = 'cyan', linewidth=1)
plt.plot(T_array    , B_N2 , '.', color = 'g' , linewidth=3)
plt.semilogy(T_array, B_NH3, '.', color = 'b', linewidth=1)
...
# Save plots
plt.savefig("CEA-TEA-TEBS.png", dpi=300)
plt.savefig("CEA-TEA-TEBS.ps", dpi=300)
